<?php
/*
Plugin Name: This Day In History Widget
...
Description: This Widget lists and links to any posts posted in the same month, week, or day of the prior years. If you don't have any posts posted matching the criteria you selected the widget will not show at all.
Version: 3.24.31
*/
define("SMALY_VERSION", "3.24.31");
if (!defined("ABSPATH")) die('You are not allowed to call this page directly.<p>You could try starting <a href="/">here</a>.');
/**
 * SMALY Main Plugin File
 * @package SMALY
*/
/*  Copyright 2011-2024 Eli Scheetz (email: wordpress@ieonly.com)
...
class SMALY_Widget_Class extends WP_Widget {
	...
	private static $SMALYrelatebys = array('Month' => array('monthnum', 'n'), 'Week' => array('w', 'W'), 'Day' => array('day', 'j'));

	public function __construct() {
	    parent::__construct(
	        'SMALY-Widget',
	        __('This Day In History', 'text_domain'),
	...
	function widget($args, $instance) {
		...
		extract($args);
		if (!$instance['title'])
			$instance['title'] = "This Day In History";
		...
		if (!is_numeric($instance['date']))
			$instance['date'] = 0;
		if (!isset(self::$SMALYrelatebys[$instance['relateby']]))
		...
		if ($instance['number'] > 0) {
			$arr = array('showposts' => $instance['number'], 'year' => $year, self::$SMALYrelatebys[$instance['relateby']][0] => date(self::$SMALYrelatebys[$instance['relateby']][1]), 'orderby' => $instance['orderby'], 'order' => $instance['order']);
			if ($instance['relateby'] == 'Day')
				$arr[self::$SMALYrelatebys["Month"][0]] = date(self::$SMALYrelatebys["Month"][1]);
			if ($instance['all'])
	...
	function form( $instance ) {
		...
		foreach (self::$SMALYrelatebys as $Time => $arr)
...
function SMALY_set_plugin_row_meta($links_array, $plugin_file) {
	if ($plugin_file == substr(__file__, (-1 * strlen($plugin_file))) && strlen($plugin_file) > 10)
		$links_array = array_merge($links_array, array('<a target="_blank" href="https://gotmls.net/donate/?key=9714e0fc36f29e59ad7bd715886786f8">'.__( 'Donate' ).'</a>'));
	return $links_array;
}
